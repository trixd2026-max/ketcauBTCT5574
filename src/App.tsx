                ...
                {results.map(({ beam, result: r }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        {fmt(beam.stirrupSpacing, 0)} / <b>{fmt(r.shear.sSuggested, 0)}</b>
                      </td>
...
function Shear({ r, spacing, onApplyS }: { r: BeamResult; spacing: number; onApplyS?: (s: number) => void }) {
  const s = r.shear;
  const needTighter = spacing > s.sSuggested + 0.5;
  return (
    <section className="result-section">
      <div className="section-heading"><h3>Cắt</h3><Status pass={s.check.pass} /></div>
      <Result label="ho" value={`${fmt(s.ho, 0)} mm`} />
      <Result label="Q / Qbt" value={`${fmt(s.qDemand)} / ${fmt(s.qbt)} kN`} />
      <Result label="Qb" value={`${fmt(s.qB)} kN`} />
      <Result label="Qsw" value={`${fmt(s.qSw)} kN`} />
      <Result label="Qb + Qsw" value={`${fmt(s.qResistance)} kN`} />
      <Result label="Asw" value={`${fmt(s.stirrupArea, 1)} mm²`} />
      <Result label="qsw" value={`${fmt(s.qsw, 2)} N/mm`} />
      <Result label="s / s,max" value={`${fmt(spacing, 0)} / ${fmt(s.sMax, 0)} mm`} />
      <Result label="s cần / gợi ý" value={`${fmt(s.sRequired, 0)} / ${fmt(s.sSuggested, 0)} mm`} />
      {onApplyS && needTighter && (
        <button type="button" className="primary" style={{ marginTop: 8, width: '100%' }}
...
function Checks({ checks }: { checks: { pass: boolean; message: string }[] }) {
  return (
    <div className="checks">
      {checks.map((c) => (
        <div key={c.message} className={c.pass ? 'text-pass' : 'text-fail'}>{c.pass ? '✓' : '×'} {c.message}</div>
      ))}
    </div>
  );
}
