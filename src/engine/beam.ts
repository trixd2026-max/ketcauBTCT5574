...
export function calcBeam(input: BeamInput): BeamResult {
  ...
  const qBFull = Math.min(Math.max(qBBase, qMin), qMax);
  const qB = qBFull * (qsw < qswMin ? qsw / qswMin : 1);
  const qSw = safe((0.75 * qsw * ho) / 1000);
  const qResistance = qB + qSw;
  const sMax = qDemand <= qMin ? Math.min(0.75 * ho, 500) : Math.min(0.5 * ho, 300);
  let sRequired = sMax;
  if (stirrupArea > 0 && stirrupStrength > 0 && ho > 0) {
    const sForQswMin = (stirrupStrength * stirrupArea) / Math.max(qswMin, 1e-6);
    if (qDemand <= qBFull) {
      sRequired = Math.min(sMax, sForQswMin);
    } else {
      const qSwNeed = Math.max(qDemand - qBFull, 0.01);
      const sForQ = (0.75 * stirrupStrength * stirrupArea * ho) / (1000 * qSwNeed);
      sRequired = Math.min(sMax, sForQswMin, sForQ);
    }
  }
  const step = sRequired >= 200 ? 50 : 25;
  const sSuggested = Math.max(step, Math.floor(sRequired / step) * step);
  ...
  const deflection = calcDeflection({ ...crackInput, L: input.L ?? 0, support: input.support ?? 'simple', limitRatio: input.limitRatio ?? 250 });
  ...
  const pass = negative.check.pass && positive.check.pass && shearPass && detailing.pass && crack.pass && ((input.L ?? 0) <= 0 || deflection.pass);
  return {
    negative, positive,
    shear: { ho, qDemand, qbt, qB, qSw, qResistance, qsw, stirrupArea, sMax, sRequired: round(sRequired, 1), sSuggested: round(sSuggested, 0), compressionCheck, resistanceCheck, spacingCheck, check: check(shearPass, shearPass ? 'Đạt cắt' : 'Không đạt cắt') },
    detailing, crack, deflection, pass, warnings,
  };
}

export const createDefaultBeam = (id: string = crypto.randomUUID()): BeamInput => ({
  id, name: 'Dầm mới', b: 300, h: 600, aTop: 50, aBottom: 50, MNegative: 120, MPositive: 100, Q: 80,
  concrete: 'B25', steel: 'CB400-V', stirrupSteel: 'CB240-T', AsTop: 942, AsBottom: 942,
  barsTop: '3d20', barsBottom: '3d20', nBarsTop: 3, nBarsBottom: 3, barDiaTop: 20, barDiaBottom: 20,
  stirrupLegs: 2, stirrupDia: 10, stirrupSpacing: 150, L: 6, humidity: 'mid', support: 'simple',
});

/** Gợi ý bố trí thép dầm từ AsRequired (mm²) và bề rộng b (mm). */
export type BarSuggestion = {
  label: string; n: number; dia: number; As: number; clearSpacing: number; ok: boolean;
};
const COMMON_DIAS = [12, 14, 16, 18, 20, 22, 25, 28, 32];
export function suggestBeamBars(AsRequired: number, b: number, a: number = 50, maxOptions = 5): BarSuggestion[] {
  const need = Math.max(AsRequired, 0);
  if (need <= 0 || b <= 0) return [];
  const out: BarSuggestion[] = [];
  for (const dia of COMMON_DIAS) {
    const a1 = (Math.PI * dia * dia) / 4;
    let n = Math.max(2, Math.ceil(need / a1 - 1e-9));
    if (n > 8) continue;
    for (const nn of [n, n + 1]) {
      if (nn < 2 || nn > 8) continue;
      const As = nn * a1;
      if (As + 1e-6 < need) continue;
      const clear = nn > 1 ? (b - 2 * a - nn * dia) / (nn - 1) : 999;
      const ok = clear >= 25 || nn <= 1;
      const label = `${nn}d${dia}`;
      if (out.some((x) => x.label === label)) continue;
      out.push({ label, n: nn, dia, As: Math.round(As * 10) / 10, clearSpacing: Math.round(clear * 10) / 10, ok });
    }
  }
  out.sort((x, y) => {
    if (x.ok !== y.ok) return x.ok ? -1 : 1;
    const ox = x.As / Math.max(need, 1);
    const oy = y.As / Math.max(need, 1);
    return ox - oy || x.n - y.n;
  });
