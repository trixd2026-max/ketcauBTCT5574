...
export type BeamResult = {
  ...
  shear: {
    ...
    /** Khoảng cách đai cần (mm) để Q ≤ Qb+Qsw với đai hiện tại */
    sRequired: number;
    /** Gợi ý s làm tròn xuống bội 50 mm, ≤ sMax */
    sSuggested: number;
...
export function calcBeam(input: BeamInput): BeamResult {
  ...
  // Gợi ý s đai: Qb lấy đủ (qsw ≥ qswMin); s ≤ Asw·Rsw/qswMin; s ≤ sMax
  let sRequired = sMax;
  ...
  return {
    ...
    shear: {
      ...
      sRequired: round(sRequired, 1),
      sSuggested: round(sSuggested, 0),
